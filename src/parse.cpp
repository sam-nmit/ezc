...
Parser::Parser(Tokenizer* tk) { this->tk = tk; }
...
std::string Parser::ParseComplexString() {
  ...
  while (this->tk->Next(&t) && t.type != TokenType::NEWLINE &&
...
std::string Parser::ParseFileSearch() {
  ezc::Token t;
  std::string s;
  ...
  while (this->tk->Next(&t)) {
    switch (t.type) {
      case ezc::TokenType::ASSIGNMENT:
        if (!this->LastIdentifier(&temp_t) ||
            !(temp_t.type == ezc::TokenType::IDENTIFIER)) {
          std::cout << t.position << ": "
                    << "Assignment must be used with a identifer."
                    << temp_t.value << std::endl;
          return;
        } else {
    ...
    case ezc::TokenType::EXECUTE:
        std::string cmd = this->ParseComplexString();
